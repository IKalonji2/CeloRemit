...
def create_wallet(api_key):
    ...
    conn.request("GET", "/v3/celo/wallet", headers=headers)
...
def create_virtual_currency(api_key):
    conn = http.client.HTTPSConnection("api-eu1.tatum.io")
    payload = "{\"name\":\"VC_cUSD\",\"supply\":\"1000000000\",\"basePair\":\"USD\",\"baseRate\":1,\"customer\":{\"accountingCurrency\":\"USD\",\"customerCountry\":\"SA\",\"externalId\":\"123654\",\"providerCountry\":\"SA\"},\"description\":\"Celo Remit cUSD\",\"accountCode\":\"Main_Account\",\"accountNumber\":\"1234567890\",\"accountingCurrency\":\"USD\"}"
    headers = {
...
def get_virtual_currency(api_key):
    conn = http.client.HTTPSConnection("api-eu1.tatum.io")
    headers = { 'x-api-key': api_key }
    conn.request("GET", "/v3/ledger/virtualCurrency/VC_cUSD", headers=headers)
    res = conn.getresponse()
    data = res.read()
    return json.loads(data.decode("utf-8"))